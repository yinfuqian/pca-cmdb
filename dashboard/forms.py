...
class RegistrationForm(BootstrapForm):
    username = forms.CharField(label='用户名', max_length=50, error_messages={"required": "用户名不能为空！"},
                               widget=widgets.TextInput(attrs={'placeholder': '请输入用户名/电话'}))
    ...
    def clean_username(self):
        ...
        if len(username) < 6:
            ...
            filter_result = Account.objects.filter(username__exact=username)  # exact精准匹配
...
class LoginForm(BootstrapForm):
    ...
    def clean_username(self):
        ...
        if phone_check(username):
            filter_result = Account.objects.filter(tel__exact=username)
